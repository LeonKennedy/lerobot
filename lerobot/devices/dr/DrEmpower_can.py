...
class DrEmpower_can(object):
    ...
    def __init__(self, com='', uart_baudrate=115200):
        """初始化函数。
    ...
    def set_id(self, id_num=0, new_id=0):
        ...
        """
        try:
            self.write_property(id_num=id_num, property='dr.config.can_id', value=new_id)
            if self.read_property(id_num=new_id, property='dr.config.can_id') == new_id:
    ...
    def set_zero_position(self, id_num=0):
        ...
        try:
            ...
            time.sleep(1)
            order_num = 0x24  # 使用 write_to_flash 保存零点偏移量
            data = self.format_data([order_num], 'u32', 'encode')
            self.send_command(id_num=id_num, cmd=0x08, data=data, rtr=0)  # 需要用标准帧（数据帧）进行发送，不能用远程帧
            print("零点设置成功！")
        except Exception as e:
        ...
        try:
            if len(id_list) == len(angle_list):
                if mode == 0:
                    ...
                    if len(self.cur_angle_list) != len(angle_list) or self.set_angles_mode_1_flag == 0 or self.estop_flag == 1:
                        self.cur_angle_list = []
                        if self.angle_speed_torque_state_flag == 0:
                            for i in range(len(id_list)):
                                state = self.get_angle(id_num=id_list[i])
                                if READ_FLAG == 1:
                                    self.cur_angle_list.append(state)
                                else:
                    ...
                    if speed > 0 and param > 0:
                        ...
                        order_num = 0x11
                        data = self.format_data([order_num, 0], 'u32 u16', 'encode')  # 将order_num转换成占4字节的数据，
                        self.send_command(id_num=0, cmd=0x08, data=data, rtr=0)  # 需要用标准帧（数据帧）进行发送，不能用远程帧
                    else:
                        print("speed or accel <= 0")
                    self.set_angles_mode_1_flag = 1
                elif mode == 2:
                    for i in range(len(id_list)):
                        self.preset_angle(id_num=id_list[i], angle=angle_list[i], t=speed, param=param, mode=mode)
                    order_num = 0x12
                    data = self.format_data([order_num, 0], 'u32 u16', 'encode')
                    self.send_command(id_num=0, cmd=0x08, data=data, rtr=0)  # 需要用标准帧（数据帧）进行发送，不能用远程帧
                # self.cur_angle_list = angle_list.copy()
            else:
    ...
    def step_angle(self, id_num=1, angle=0, speed=0, param=0, mode=0):
        ...
        try:
            if mode == 0:
                ...
                order_num = 0x10
                data = self.format_data([order_num, 1], 'u32 u16', 'encode')  # # 将 order_num 转换成占4字节的数据，
                self.send_command(id_num=id_num, cmd=0x08, data=data, rtr=0)  # 需要用标准帧（数据帧）进行发送，不能用远程帧
            elif mode == 1:
                if speed > 0 and param > 0:
                    f_angle = angle
    ...
    def set_angle_adaptive(self, id_num=0, angle=0, speed=0, torque=0):
        ...
        try:
            ...
            data = self.format_data([f_angle, s16_speed, s16_torque], 'f s16 s16', 'encode')
            self.send_command(id_num=id_num, cmd=0x0B, data=data, rtr=0)
        except Exception as e:
    ...
    def impedance_control(self, id_num=0, angle=0, speed=0, tff=0, kp=0, kd=0):
        ...
        try:
            ...
            if kp != 0:
                ...
                return False
            self.preset_angle(id_num=id_num, angle=angle_set, t=speed, param=tff, mode=2)
            order_num = 0x15
    ...
    def impedance_control_multi(self, id_list=[1, 2, 3], angle_list=[0, 0, 0], speed_list=[0, 0, 0], tff_list=[0, 0, 0],
        ...
        try:
            lenth = len(id_list)
            if len(angle_list) != lenth or len(speed_list) != lenth or len(tff_list) != lenth or len(
                    kp_list) != lenth or len(kd_list) != lenth:
    ...
    def motion_aid(self, id_num=0, angle=0, speed=0, angle_err=0, speed_err=0, torque=0):
        ...
        try:
            ...
            data = self.format_data(
                [int(angle / factor), int(angle_err / factor), int(speed_err / factor), int(torque / factor)],
                's16 u16 u16 s16', 'encode')
            self.send_command(id_num=id_num, cmd=0x0D, data=data, rtr=0)  # 需要用标准帧（数据帧）进行发送，不能用远程帧
            self.set_speed_adaptive(id_num=id_num, speed_adaptive=speed)
        except Exception as e:
        ...
        try:
            ...
            for i in range(length):
                if angle_list[i] < -300 or angle_list[i] > 300:
                    ...
                    return False
            if len(angle_err_list) != length or len(speed_list) != length or len(speed_err_list) != length or len(
                    torque_list) != length:
                print("关节数量与参数数量不一致，请检查")
            for i in range(length):
                data = self.format_data(
            ...
            for i in range(length):
                ...
                self.set_speed_adaptive(id_list[i], speed_adaptive=speed_list[i])
        except Exception as e:
    ...
    def position_done(self, id_num=0):
        ...
        try:
            while kk == 0:
                kk = self.read_property(id_num, property='dr.controller.position_done')
                # print(kk)
                # print(id_num)
        except Exception as e:
    ...
    def positions_done(self, id_list=[1, 2, 3]):
        ...
        try:
            while (k == 1):
                kk = 1
                for i in range(len(id_list)):
                    kk = kk & self.position_done(id_list[i])
                if kk == 1:
                    k = 0
        except Exception as e:
    ...
    def get_id(self):
        ...
        """
        try:
            id = self.read_property(id_num=0, property='dr.config.can_id')
            print(id)
            return id
        except Exception as e:
    ...
    def get_ids(self):
        ...
        try:
            ...
            self.send_command(id_num=0, cmd=0x1E, data=data, rtr=0)  # 需要用标准帧（数据帧）进行发送，不能用远程帧
            id_list = self.read_data_id()
            id_list.sort()
    ...
    def get_angle(self, id_num=0):
        ...
        Raises：
            “---error in self.get_angle---”

        """
        try:
            return self.read_property(id_num=id_num, property='dr.output_shaft.angle')
        except Exception as e:
            print("---error in self.get_angle---：", e)
            return False

    # 读取当前转速
    def get_speed(self, id_num=0):
        """读取一体化关节当前转速（r/min）。
        ...
        """
        try:
            return self.read_property(id_num=id_num, property='dr.output_shaft.speed')
        except Exception as e:
    ...
    def get_state(self, id_num=0):
        ...
        try:
            return [self.read_property(id_num=id_num, property='dr.output_shaft.angle'),
                    self.read_property(id_num=id_num, property='dr.output_shaft.speed')]
        except Exception as e:
            print("---error in get_state---：", e)
            return False

    # 读取力矩大小及方向
    def get_torque(self, id_num=0):
        """读取一体化关节当前力矩（Nm）。
        ...
        """
        try:
            return self.read_property(id_num=id_num, property='dr.output_shaft.torque')
        except Exception as e:
            print("---error in get_torque---：", e)
            return False

    # 读取一体化关节当前电压及电流
    def get_vol_cur(self, id_num=0):
        """读取一体化关节的当前电压和电流。
        ...
        vol_cur = [0, 0]
        try:
            vol_cur[0] = self.read_property(id_num=id_num, property='dr.voltage')
            vol_cur[1] = self.read_property(id_num=id_num, property='dr.motor.Iq_measured')
            return vol_cur
        except Exception as e:
    ...
    # 开启角度、转速、力矩实时反馈函数
    def enable_angle_speed_torque_state(self, id_num=0):
        """开启角度、转速、力矩实时反馈函数.
        ...
        try:
            self.write_property(id_num, 'dr.can.enable_state_feedback', 1)
            self.angle_speed_torque_state_flag = 1
            return True
        except Exception as e:
    ...
    def set_state_feedback_rate_ms(self, id_num=0, n_ms=2):
        ...
        try:
            for i in range(5):
                self.write_property(id_num, 'dr.config.state_feedback_rate_ms', n_ms)
            return True
        except Exception as e:
    ...
    # 角度、转速、力矩实时反馈函数
    def angle_speed_torque_state(self, id_num=1, n=1):
            ...
            if READ_FLAG == 1:
                for i in range(n):
                    ...
                    if id_num == (cdata[1] * 256 + cdata[2] - 1) >> 5:
                        angle_speed_torque = self.format_data(data=cdata[3:], format='f s16 s16', type='decode')
                        return [round(angle_speed_torque[0], 3), round(angle_speed_torque[1] * 0.01, 3),
    ...
    # 取消角度、转速、力矩实时反馈函数
    def disable_angle_speed_torque_state(self, id_num=0):
        """取消角度、转速、力矩实时反馈函数.
        ...
        try:
            for i in range(5):
                self.write_property(id_num, 'dr.can.enable_state_feedback', 0)
            time.sleep(0.5)
            self.angle_speed_torque_state_flag = 0
            while self.uart.inWaiting() > 0:
                self.uart.read(1)  # 清空串口中残余的数据
            return True
        except Exception as e:
    ...
    def get_angle_speed_torque_all(self, id_list=[1, 2, 3]):
    ...
    def get_pid(self, id_num=0):
        ...
        """
        try:
            P = self.read_property(id_num=id_num, property='dr.controller.config.angle_gain')
            I = self.read_property(id_num=id_num, property='dr.controller.config.speed_integrator_gain')
            D = self.read_property(id_num=id_num, property='dr.controller.config.speed_gain')
            print('位置增益 P：%s' % P)
            print('积分增益 I：%s' % I)
            print('转速增益 D：%s' % D)
        except Exception as e:
    ...
    def read_property(self, id_num=0, property=''):
        ...
        try:
            ...
            print("---error in self.read_property---：", e)
            return False
    ...
    def set_angle_range(self, id_num=0, angle_min=-180, angle_max=180):
        ...
        """
        try:
            angle = self.get_angle(id_num=id_num)
            if READ_FLAG == 1:
                if angle >= angle_min and angle <= angle_max:
                    self.write_property(id_num=id_num, property='dr.output_shaft.angle_min', value=angle_min)
    ...
    def disable_angle_range(self, id_num=0):
        ...
        """
        try:
            self.write_property(id_num=id_num, property='dr.output_shaft.enable_angle_limit', value=0)
            bol = self.read_property(id_num=id_num, property='dr.output_shaft.enable_angle_limit')
            if bol == 0:
    ...
    def disable_angle_range_config(self, id_num=0):
        ...
        """
        try:
            self.write_property(id_num=id_num, property='dr.config.enable_angle_limit', value=0)
            bol = self.read_property(id_num=id_num, property='dr.config.enable_angle_limit')
            # self.save_config(id_num=id_num)
            if bol == 0:
                print("取消角度限位属性成功")
            else:
    ...
    def set_mode(self, id_num=0, mode=2):
        ...
        try:
            if mode == 1:
                self.write_property(id_num=id_num, property='dr.requested_state', value=1)
            elif mode == 2:
                self.write_property(id_num=id_num, property='dr.requested_state', value=2)
        except Exception as e:
